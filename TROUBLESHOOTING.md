...
- **High restart count in PM2:** A large restart count on `vps-mcp` is expected — each `deploy_vps_mcp` call restarts the process. This is not indicative of instability. Concern on