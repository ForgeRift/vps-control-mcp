...
| ID | Fix | Verification |
| --- | --- | --- |
| F-OP-85 | `validateServiceArgs`: changed `args[args.length-1]` to `args[1]`; added `args.length > 2` guard; added `name.startsWith('-')` flag-injection check. Service RED pattern extended with `force-reload\|try-restart\|condrestart\|force-stop`. | `service nginx status` passes; `service nginx restart`, `service -H nginx status`, `service nginx status extra` all blocked. |
| F-OP-86 | `validateSystemctlArgs`: added pre-loop over args checking for `-H`, `--host*`, `-M`, `--machine*` before sub-command dispatch. HARD_BLOCKED pattern `/\bsystemctl\b[^|&;\n]*(-H\b\|--host[=\s]\|--machine[=\s]\|-M\b)/i` added to `ssh-tunnel-pivot` category. | `systemctl -H remote status nginx` blocked at both validator and HARD_BLOCKED layers. |
| F-OP-87 | `validateSystemctlArgs` READ_ONLY set: removed `show` and `cat`. RED pattern extended to include `show\|cat`. | `systemctl show nginx`, `systemctl cat nginx.service` blocked at both validator and RED-pattern layers. `systemctl status nginx` still passes. |
| F-OP-88 | `validateCrontabArgs`: removed `args.length === 0` branch — only `args.length === 1 && args[0] === '-l'` now passes. | Bare `crontab` blocked; `crontab -l` passes. |
| F-OP-89 | `validatePm2Args` READ_ONLY: removed `reload`. | `pm2 reload` blocked by validator (AMBER pattern still fires for warning). |
| F-OP-90 | `validatePm2Args` READ_ONLY: removed `flush`. HARD_BLOCKED pattern `/\bpm2\s+flush\b/i` added to `audit-log-destruction` category. | `pm2 flush` blocked at both layers. |
| F-OP-91 | `findPm2Log`: replaced `path.resolve(p)` with `fs.realpathSync(p)` inside try/catch (returns `false` on error). | Symlink escape via PM2_LOG_DIR symlink no longer bypasses bounds check. |
| F-OP-92 | `validateDigArgs`: added `@`-prefix blocking and AXFR/IXFR query-type blocking. New `validateHostArgs`: blocks `-l`/`--list`. New `validateNslookupArgs`: requires `args.length >= 1`. Both new validators wired into POSITIVE_ALLOWLIST. | `dig @8.8.8.8 google.com`, `dig google.com AXFR`, `host -l google.com`, bare `nslookup` all blocked. |
| F-OP-93 | Crontab RED pattern: `/-[eru]\b/` extended to `/(-[eru]\b\|--edit\b\|--remove\b\|--user\b)/`. | `crontab --edit`, `crontab --remove`, `crontab --user root` all blocked. |
| F-OP-94 | Systemctl RED pattern: added `set-environment\|unset-environment\|import-environment\|link\|revert\|switch-root\|freeze\|thaw\|show\|cat` (also closes F-OP-87 at RED layer). | All new sub-commands blocked at RED-pattern layer. |
| F-OP-95 | `atq` allowlist entry: `allowAny` replaced with `allowFlags('-V', '--version', '-q')`. | `atq`, `atq -V` pass; `atq -f /etc/passwd` blocked. |
| F-OP-96 | `readAuditLog`: added runtime loop over `SENSITIVE_FILE_PATTERNS` (excluding `APP_DIR_ROOT_CARVEOUT`) on `CONFIG.AUDIT_LOG_PATH` before `fs.existsSync`. Returns `BLOCKED` string if matched. | Misconfigured `AUDIT_LOG_PATH=/etc/shadow` would return blocked message rather than file contents. |
| F-OP-97 | Deferred. Low practical impact. Tracked for future maintenance. | — |
