...
function truncate(output) {
    if (output.length <= CONFIG.MAX_OUTPUT_CHARS)
        return output;
    return (output.slice(0, CONFIG.MAX_OUTPUT_CHARS) +
...
function validatePath(filePath) {
    ...
    if (!allowed) {
        throw new Error(`Path not permitted: "${filePath}". Reads are restricted to: ${ALLOWED_READ_DIRS.join(', ')}`);
    }
    return resolved;
}
function validateProcess(name) {
    if (!CONFIG.ALLOWED_PROCESSES.includes(name)) {
        throw new Error(`Process not permitted: "${name}". Allowed processes: ${CONFIG.ALLOWED_PROCESSES.join(', ')}`);
    }
}
...
function validateCommand(command) {
    for (const pattern of BLOCKED_PATTERNS) {
        if (pattern.test(command)) {
            throw new Error(`Blocked pattern detected (${pattern}). Use structured tools instead, or ask the user to run this manually.`);
        }
    }
}
...
async function getRecentErrors(processName, lines) {
...
async function readFileSection(filePath, startLine, endLine) {
...
async function searchFile(filePath, pattern, contextLines) {
    const safePath = validatePath(filePath);
    ...
    try {
        ...
        const e = err;
        if (e.code === 1)
            return `No matches found for pattern "${pattern}" in ${path.basename(safePath)}.`;
        throw err;
    }
}
...
async function gitPush(dryRun, description) {
    if (dryRun) {
        return [
            'DRY RUN — nothing executed.',
...
async function restartProcess(processName, dryRun) {
    validateProcess(processName);
    if (dryRun) {
        return [
            'DRY RUN — nothing executed.',
...
async function runApprovedCommand(command, justification, dryRun) {
    ...
    if (customCommandCount > CONFIG.MAX_CUSTOM_COMMANDS_PER_SESSION) {
        throw new Error(`Custom command session limit (${CONFIG.MAX_CUSTOM_COMMANDS_PER_SESSION}) reached. ` +
...
// ─── Deploy Tools ──────────────────────────────────────────────────────────────────
async function deploySharpEdge(dryRun, description) {
    if (!description || description.trim().length < 5) {
    ...
    if (dryRun) {
        const stepList = steps.map((s, i) => '  ' + (i + 1) + '. ' + s.label + (s.cwd ? ' (cwd: ' + s.cwd + ')' : '')).join('\n');
        return [
            'DRY RUN — nothing executed.',
    ...
    const results = ['=== SharpEdge Deploy: ' + description + ' ===', ''];
...
async function deployVpsMcp(dryRun, description) {
    if (!description || description.trim().length < 5) {
        throw new Error('description is required (min 5 chars) — describe what is being deployed.');
    }
    ...
    const results = ['=== vps-control-mcp Deploy: ' + description + ' ===', ''];
    for (const step of steps) {
        results.push('--- ' + step.label + ' ---');
        try {
            const { stdout, stderr } = await runCmd(step.cmd, step.args, step.cwd);
    ...
    try {
        switch (name) {
            ...
            case 'get_recent_errors':
                return await getRecentErrors(args.process_name, args.lines ?? 20);
            case 'read_file_section':
                return await readFileSection(args.file_path, args.start_line, args.end_line);
            case 'search_file':
                return await searchFile(args.file_path, args.pattern, args.context_lines ?? 3);
            case 'git_status':
                return await gitStatus();
            case 'git_log':
                return await gitLog(args.count ?? 10);
            case 'git_pull':
                return await gitPull(args.dry_run ?? true);
            case 'git_push':
                return await gitPush(args.dry_run ?? true, args.description ?? '');
            case 'restart_process':
                return await restartProcess(args.process_name, args.dry_run ?? true);
            case 'get_system_health':
                return await getSystemHealth();
            case 'run_approved_command':
                return await runApprovedCommand(args.command, args.justification, args.dry_run ?? true);
            case 'deploy':
                return await deploySharpEdge(args.dry_run ?? true, args.description ?? '');
            case 'deploy_vps_mcp':
                return await deployVpsMcp(args.dry_run ?? true, args.description ?? '');
            default:
