...
**Operating with `LAYER_STRICT_MODE=false` is "degraded mode."** The plugin's intended security model is the combination of Layer 1 (deterministic deny-list + per-binary validators) **and** Layers 2–3 (AI classification). Operating with only Layer 1 active is supported as an explicit, deliberate operator choice, but it is materially weaker than the combined model and ForgeRift does not represent it as equivalent. Operators choosing this configuration accept the risk of any command Layer 1 alone fails to block; ForgeRift's liability for damage arising in this configuration is limited per the [MCP EULA](https://forgerift.io/legal/mcp-eula) Sections 6.1, 10, 11, and 12. Marketing language about "AI-assisted safety classification" describes the combined model and does not accurately describe the strict-mode-off configuration.

**Deliberate weakening of safety controls is not supported.** Modifying, patching, or replacing the deterministic Layer 1 patterns (or the validators, the audit-log writer, the secret-scrubbing pipeline, or the sensitive-file matcher) to permit otherwise-blocked commands is a violation of the [MCP EULA](https://forgerift.io/legal/mcp-eula) Section 3 (Restrictions). Operators who do so void the liability protections the EULA otherwise provides. The plugin's source is available; that does not mean operating a deployed plugin with the safety controls weakened is permitted under the deployed-product license.

**Binary-name canonicalization.** As of 2026-05, command argv is normalized through a `BINARY_ALIASES` map before pattern matching: `pwsh` → `powershell`, `nodejs` → `node`, `pip3` → `pip`, `ncat`/`netcat`/`nc.openbsd` → `nc`, and similar variants resolve to their canonical forms. This closes a class of bypasses where alternative spellings of allowlisted binaries previously slipped pattern matches that targeted only the canonical name, and it complements the positive allowlist (which rejects any binary not on the canonical list outright). See `src/tools.ts` for the full alias map.
